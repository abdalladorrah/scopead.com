...
export const Frames = {
    ...
    renderHero: (data) => `
        <section id="hero" class="frame frame-hero" style="min-height: 80vh; display: flex; flex-direction: column; justify-content: center; margin-bottom: 5rem; position: relative;">
            ...
            <div style="position: relative; z-index: 10;">
                ...
                ${UI.CTAButton(data.ctaText, 'rights')}
            </div>
        </section>
    `,
    ...
    renderValues: (data) => `
        <section id="values" class="frame frame-values" style="padding-top: 5rem; margin-bottom: 5rem; border-top: 1px solid var(--border-subtle);">
            ...
            <div style="background: hsla(164, 88%, 25%, 0.05); padding: 3rem; border-radius: 16px; border: 1px solid var(--border-subtle);">
    ...
    renderStructure: (data) => `
        <section id="structure" class="frame frame-structure" style="padding-top: 5rem; margin-bottom: 5rem; border-top: 1px solid var(--border-subtle);">
            ...
            <h3 style="font-size: 1.8rem; margin: 3rem 0 1.5rem; color: var(--primary-light);"><i class="fa-solid fa-users"></i> فريق العمل </h3>
            <div class="grid-4">
                ${data.roles.map(r => UI.PersonCard("دور", r.role, r.desc, r.icon)).join('')}
            </div>

            <h3 style="font-size: 1.8rem; margin: 4rem 0 1.5rem; color: var(--primary-light);"><i class="fa-solid fa-spinner"></i> دورة العمل </h3>
                ...
                ${data.flow.map((step, index) => `
                    <div class="timeline-step">
                        ...
                        <div class="step-content glass-card">
                            <h3 style="color: var(--text-primary); margin-bottom: 0; display: flex; align-items: center; gap: 0.5rem; font-size: 1.1rem;">
                                ${step.step}
                            </h3>
                        </div>
    ...
    renderSales: (data) => `
        <section id="sales" class="frame frame-sales" style="padding-top: 5rem; margin-bottom: 5rem; border-top: 1px solid var(--border-subtle);">
            ${UI.SectionHeader(data.title, data.tag)}
            
            <h3 style="font-size: 1.8rem; margin: 3rem 0 1.5rem; color: var(--primary-light);">${data.mainHeadline}</h3>
            <p style="font-size: 1.25rem; color: var(--text-secondary); margin-bottom: 3rem; line-height: 1.8;">${data.intro}</p>

            <h3 style="font-size: 1.5rem; margin: 2rem 0 1.5rem; color: var(--accent-color);"><i class="fa-solid fa-shield-halved"></i> ${data.challengesTitle} </h3>
            <div class="grid-3" style="margin-bottom: 4rem;">
                ${data.challenges.map(c => `
                    <div class="glass-card" style="border-top: 3px solid var(--accent-color);">
                        <div style="font-size: 2rem; color: var(--border-subtle); margin-bottom: 1rem;"><i class="${c.icon}"></i></div>
                        <h4 style="color: var(--text-primary); margin-bottom: 0.5rem; font-size: 1.1rem;"><span style="color: var(--accent-color);">المشكلة:</span> ${c.problem}</h4>
                        <p style="color: var(--text-secondary); line-height: 1.6; font-size: 0.95rem;"><strong style="color: var(--primary-light);">تدخل الديجيتال:</strong> ${c.solution}</p>
                    </div>
                `).join('')}
            </div>
            
            <h3 style="font-size: 1.5rem; margin: 2rem 0 1.5rem; color: var(--accent-color);"><i class="fa-solid fa-chess-knight"></i> ${data.tacticsTitle}</h3>
            <div class="grid-2" style="margin-bottom: 4rem;">
                ${data.tactics.map(t => UI.FeatureCard(t.icon, t.title, t.desc)).join('')}
            </div>
            
            <div style="background: hsla(164, 88%, 25%, 0.05); padding: 3rem; border-radius: 16px; border: 1px solid var(--border-subtle); margin-bottom: 4rem;">
                <h3 style="font-size: 1.5rem; margin-bottom: 2rem; color: var(--primary-light);"><i class="fa-solid fa-briefcase"></i> ${data.materialsTitle}</h3>
                <div class="grid-2" style="gap: 1.5rem;">
                    ${data.materials.map(m => `
                        <div style="display: flex; align-items: flex-start; gap: 0.8rem;">
                            <i class="fa-solid fa-check" style="color: var(--accent-color); margin-top: 0.4rem;"></i>
                            <span style="font-size: 1.1rem; color: var(--text-primary);">${m}</span>
                        </div>
                    `).join('')}
                </div>
            </div>

            ${UI.CoreMessage(data.coreMessage)}
        </section>
    `,
    ...
    renderFullPage: (sectionsData) => `
        ...
        ${Frames.renderSales(sectionsData.sales)}
    `
};
